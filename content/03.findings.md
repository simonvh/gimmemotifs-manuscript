...
@doi:doi:10.1016/j.molcel.2010.05.004]. CIS-BP integrates many individual motif
...
FANTOM4 [ref], Homer [@doi:doi:10.1016/j.molcel.2010.05.004], Factorbook
[@doi:10.1093/nar/gks1221], the ENCODE motifs from Kheradpour et al [@doi:
10.1093/nar/gkt1249], HOCOMOCO [@doi:10.1093/nar/gkx1106], the RSAT clustered
motifs [@doi:10.1093/nar/gkx314] and the motif database created by Madsen et al.
[@doi:10.1101/gr.227231.117]. Figure 3A shows distribution of the ROC AUC (area
...
It has been consistently noted that there is no *de novo* motif prediction
algorithm that consistently performs well across different data sets
[@Tompa2005; other benchmark]. New approaches and algorithms for *de novo* motif
...
typically have tested only a few motif finders or used only a few datasets [@Jayaram2016].
...
files for 270 proteins from ENCODE (Supplemental Table X; [ref website]) and
...
In line with previous results [ref Tompa], there is no single tool that
...
(ROC AUC, recall at 10% FDR and MNCP[@Granek]). The results are visualized as a
...
enrichment method to compare to samples [Lesluyes2014]. The regression approach MARA [@fantom2009], as
implemented in ISMARA [@ref ISMARA] normalizes between data sets before
...
hematopoietic H3K27ac ChIP-seq experiments, mainly from BLUEPRINT [@ref
BLUEPRINT] (Supplemental Table X). We determined the number of H3K27ac reads per
...
coordinate descent [@lightning] and regression with boosted trees (XGBoost ref).
The coefficients or feature importances were ranked and combined using rank
aggregation [@ref]. A p-value was calculated for consistentily high ranking and
...
required to generate mature T and B cells [@Pioli2015;@Pioli2016] in mice. ZEB1
is expressed in T cells and represses expression of IL-2 [@Wang2009], as well as
other immune genes such as CD4 [@Brabletz1999] and GATA3 [@Gregoire1999]. ZEB1
knockout mice exhibit a defect in thymocyte development [@Takagi1998]. Together,
...
endothelial proliferation and angiogenesis [@Kohler2011].
